...
seqtk subseq data/GRCh38_no_alt_analysis_set.fasta chr16X.bed|seqtk seq -c -M test_somalier_small.bed | sed '/^>/ s/:.*//' | cat - extra_chr.fa | pigz -p 36 > hg38.test.fa.gz

# Subsample to reduce size
samtools view -@ 36 -s 0.8 -M -L test_somalier_small.bed data/HG002_haplotagged.bam -h -O BAM | samtools calmd -b - data/GRCh38_no_alt_analysis_set.fasta > hg002_somalier_small_revio.bam
samtools view -@ 36 -s 0.8 -M -L test_somalier_small.bed data/HG003_haplotagged.bam -h -O BAM | samtools calmd -b - data/GRCh38_no_alt_analysis_set.fasta > hg003_somalier_small_revio.bam
samtools view -@ 36 -s 0.8 -M -L test_somalier_small.bed data/HG004_haplotagged.bam -h -O BAM | samtools calmd -b - data/GRCh38_no_alt_analysis_set.fasta > hg004_somalier_small_revio.bam

# samtools view -@ 36 -s 0.5 -M -L test_somalier_small.bed data/hg002.haplotagged.bam -h -O BAM | samtools calmd -b - data/GRCh38_no_alt_analysis_set.fasta > hg002_somalier_small_ont.bam
...
bcftools view -R test_somalier_small.bed  data/CoLoRSdb.GRCh38.v1.0.0.pbsv.jasmine.vcf.gz --output-type z --output colorsdb.test_data.vcf.gz

# 7. Copy files, and make one copy smaller
cp colorsdb.test_data.vcf.gz reference/colorsdb.test_data.vcf.gz
cp hg38.test.fa.gz reference/hg38.test.fa.gz
cp test_somalier_small.bed reference/test_data.bed

cp hg002_somalier_small_revio.bam testdata/HG002_PacBio_Revio.bam
cp hg003_somalier_small_revio.bam testdata/HG003_PacBio_Revio.bam
cp hg004_somalier_small_revio.bam testdata/HG004_PacBio_Revio.bam
...
cp hg002_somalier_small_revio.fastq.gz testdata/HG002_PacBio_Revio.fastq.gz
samtools view -@ 36 -s 0.01 -h -O BAM hg002_somalier_small_revio.bam > testdata/HG002_PacBio_Revio_copy.bam
samtools index testdata/HG002_PacBio_Revio_copy.bam

cp hg002_somalier_small_ont.bam testdata/HG002_ONT.bam
samtools index testdata/HG002_ONT.bam
samtools view -@ 36 -s 0.01 -h -O BAM hg002_somalier_small_ont.bam > testdata/HG002_ONT_copy.bam
samtools index testdata/HG002_ONT_copy.bam
cp hg002_somalier_small_ont.fastq.gz testdata/HG002_ONT.fastq.gz